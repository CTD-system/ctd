...
import { plantillasService, type Plantilla, type Bloque } from "@/src/lib/plantillas";
...
export function EditPlantillaPage({ plantilla, onSuccess }: EditPlantillaPageProps) {
  ...
  // Cargar datos de la plantilla al iniciar
  useEffect(() => {
    setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.response?.data?.message || "Error al actualizar plantilla",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const agregarBloque = (tipo: Bloque["tipo"]) => {
    ...
    setFormData({
      ...formData,
      estructura: { ...formData.estructura, bloques: [...formData.estructura.bloques, nuevoBloque] },
    });
  };
  // 🟢 Eliminar una fila específica de la tabla
...
// 🟢 Eliminar una columna específica (de encabezados y filas)
const eliminarColumnaTabla = (
  bloque: Bloque,
  columnaIndex: number,
  bloqueIndex: number
) => {
  // Solo aplica si es una tabla
  if (bloque.tipo !== "tabla") return;

  // Eliminar columna de encabezados
  const nuevosEncabezados = bloque.encabezados.filter((_, i) => i !== columnaIndex);

  // Eliminar esa columna en cada fila
  const nuevasFilas = bloque.filas.map((fila) =>
    fila.filter((_, i) => i !== columnaIndex)
  );

  // Actualizar el bloque de la tabla en el estado principal
  actualizarBloque(bloqueIndex, {
    ...bloque,
  ...
  const actualizarBloque = (index: number, bloqueActualizado: Bloque) => {
    ...
    setFormData({ ...formData, estructura: { ...formData.estructura, bloques: nuevosBloques } });
  };
  ...
  const agregarColumnaTabla = (bloque: Bloque, index: number) => {
    if (bloque.tipo === "tabla") {
      const nuevosEncabezados = [...bloque.encabezados, `Columna ${bloque.encabezados.length + 1}`];
      const nuevasFilas = bloque.filas.map((fila) => [...fila, ""]);
      actualizarBloque(index, { ...bloque, encabezados: nuevosEncabezados, filas: nuevasFilas });
    }
  };

  const actualizarCeldaTabla = (bloque: Bloque, row: number, col: number, value: string, index: number) => {
    if (bloque.tipo === "tabla") {
      const nuevasFilas = bloque.filas.map((fila, r) => (r === row ? fila.map((celda, c) => (c === col ? value : celda)) : fila));
      actualizarBloque(index, { ...bloque, filas: nuevasFilas });
    }
  };

  // ======== Renderizado recursivo editable ========
  const renderBloqueEditable = (bloque: Bloque, index: number, nivel = 0): React.ReactNode => {
    switch (bloque.tipo) {
      case "capitulo":
      case "subcapitulo":
        return (
          <div key={index} className="mb-4" style={{ marginLeft: `${nivel * 20}px` }}>
            <Input
              placeholder="Título"
              value={bloque.titulo}
              onChange={(e) => actualizarBloque(index, { ...bloque, titulo: e.target.value } as Bloque)}
            />
            {bloque.bloques?.map((subBloque, subIndex) => renderBloqueEditable(subBloque, subIndex, nivel + 1))}
          </div>
        );
     ...
     case "tabla":
  return (
    <div key={index} className="mb-4 overflow-x-auto" style={{ marginLeft: `${nivel * 20}px` }}>
      <div className="flex gap-2 mb-2">
        <Button size="sm" variant="outline" type="button" onClick={() => agregarFilaTabla(index)}>
          Agregar Fila
        </Button>
        <Button size="sm" variant="outline" type="button" onClick={() => agregarColumnaTabla(bloque, index)}>
          Agregar Columna
        </Button>
      </div>
      ...
      <table className="w-full border border-gray-300">
        ...
        <thead>
          <tr>
            {bloque.encabezados.map((enc:any, ci) => {
              // Determinar el texto del encabezado (soporta string o { text })
              ...
              return (
                <th key={ci} className="border p- relative">
                  <Input
                    className="w-full"
                    ...
                    onChange={(e) => {
                      ...
                      if (typeof enc === "object" && enc !== null) {
                        nuevosEncabezados[ci] = { ...enc, text: e.target.value };
                      } else {
                        nuevosEncabezados[ci] = e.target.value;
                      }
                      actualizarBloque(index, { ...bloque, encabezados: nuevosEncabezados });
                    }}
                  />
                  <button
                  type="button"
          ...
          {bloque.filas.map((fila, ri) => (
            <tr key={ri}>
              {fila.map((celda:any, ci) => {
                // Determinar el texto de la celda (soporta string o { text })
                const valorCelda =
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const nuevasFilas = bloque.filas.map((f, r) =>
                          r === ri
                            ? f.map((c, cIndex) => (cIndex === ci ? nuevoValor : c))
                            : f
                        );
                        actualizarBloque(index, { ...bloque, filas: nuevasFilas });
                      }}
      ...
      case "imagen":
        return (
          <Input
            ...
            value={bloque.src}
            onChange={(e) => actualizarBloque(index, { ...bloque, src: e.target.value } as Bloque)}
            className="mb-3"
          />
        );
      ...
      case "placeholder":
        return (
          <Input
            ...
            value={bloque.clave}
            onChange={(e) => actualizarBloque(index, { ...bloque, clave: e.target.value } as Bloque)}
            className="mb-3"
          />
        );
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white/80 p-6 rounded shadow space-y-4">
          ...
          <div className="space-y-4">
            {formData.estructura.bloques.map((bloque, index) => renderBloqueEditable(bloque, index))}
          </div>
        </div>
